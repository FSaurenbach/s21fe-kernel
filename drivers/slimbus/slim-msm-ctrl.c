...
static struct msm_slim_sat *msm_slim_alloc_sat(struct msm_slim_ctrl *dev)
{
	...
	sat->dev = dev;
	scnprintf(name, SLIMBUS_NAME_SIZE, "msm_sat%d", dev->nsats);
	sat->satcl.name = name;
...
static void msm_slim_hw_init(struct msm_slim_ctrl *dev)
{
...
static int msm_slim_probe(struct platform_device *pdev)
{
	...
	/* Hardware register initialisation */
	msm_slim_hw_init(dev);
